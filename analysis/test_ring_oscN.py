...
async def worker(lane, jobs, queue, pdk_path, tmpdir):
    with tempfile.TemporaryDirectory(prefix='lane_', dir=tmpdir) as lanedir:
        ...
        global spice_filename
        shutil.copy(spice_filename, lanedir)

        prefix_path = os.path.join(lanedir, 'prefix.spice')
        out_path = os.path.join(lanedir, 'out')
        vco_current_path = os.path.join(out_path, 'ring_osc_current.txt')
        vco_fft_path = os.path.join(out_path, 'ring_osc_fft.txt')
        ...
        for config in jobs:
            ...
            try:
                ...
                # run spice
                proc = await asyncio.create_subprocess_shell(f'ngspice -b -a {spice_filename}',
                    cwd=lanedir,
...
async def main():
    ...
    parser.add_argument('--osc', help='Ring Oscillator Size', type=int, required=True)
    ...
    global spice_filename
    spice_filename = f'test_ring_osc{args.osc}.spice'
