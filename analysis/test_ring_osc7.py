...
async def worker(lane, jobs, queue, pdk_path, tmpdir):
    with tempfile.TemporaryDirectory(prefix='lane_', dir=tmpdir) as lanedir:
        ...
        shutil.copy('test_ring_osc7.spice', lanedir)

        prefix_path = os.path.join(lanedir, 'prefix.spice')
        out_path = os.path.join(lanedir, 'out')
        vco_current_path = os.path.join(out_path, 'ring_osc7_current.txt')
        vco_fft_path = os.path.join(out_path, 'ring_osc7_fft.txt')
        ...
        for config in jobs:
            ...
            try:
                ...
                # run spice
                proc = await asyncio.create_subprocess_shell('ngspice -b -a test_ring_osc7.spice',
                    cwd=lanedir,
